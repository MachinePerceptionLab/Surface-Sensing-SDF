...
def plot(implicit_network, indices, plot_data, path, epoch, img_res, plot_nimgs, resolution, grid_boundary,  level=0):
    ...
    if plot_data is not None:
        ...
        # plot ncc maps
        if plot_data['ncc_loss'] !=None:
            plot_ncc_maps(plot_data['ncc_loss'], plot_data['rgb_gt'], path, epoch, plot_nimgs, img_res, indices)
...
def plot_ncc_maps(rgb_points, ground_true, path, epoch, plot_nrow, img_res, indices, exposure=False):
    ground_true = ground_true.cuda()

    output_vs_gt = torch.cat((rgb_points.expand(-1,-1,3), ground_true), dim=0)
    output_vs_gt_plot = lin2img(output_vs_gt, img_res)
    ...
    if exposure:
        ...
        img.save('{0}/ncc_{1}_{2}.png'.format(path, epoch, indices[0]))
