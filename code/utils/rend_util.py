...
def save_pointcloud(pointcloud = None, path="",c =[0.,0.,0.]):
    if isinstance(pointcloud, torch.Tensor):
        pointcloud = pointcloud.detach().cpu()
    vertices = pointcloud.view(-1,3)
    pc = trimesh.points.PointCloud(vertices.numpy(),colors=torch.tensor(c)[None].repeat(vertices.shape[0],1))
    pc.export(path)
...
def world2cam(points ,pose):
    pose = pose.squeeze()
    cam_pose = pose[:3,:3].T
    cam_pos = pose[:3,3]
    world_translate = -1* cam_pose @ cam_pos
    w2c = torch.eye(4).cuda()
    w2c[:3,:3] = cam_pose
    w2c[:3,3] = world_translate

    points = points.squeeze()
    # project and filter
    vertices = torch.cat((points, torch.ones_like(points[:, :1])), dim=-1)
    vertices = vertices.permute(1, 0)
    vertices = vertices.float()
    points_cam= (w2c @ vertices).permute(1,0)[:,:3] #[1024,3]
...
def convert_gtdepth(model_input, ground_truth):
    ...
    surface_points = get_surface_point(uv, depth_pose, depth_intrinsic,gt_depth_gt.squeeze()[None]).reshape(-1, 3) #[1024,3]
    surface_points = (torch.linalg.inv(scale_mat[:3, :3]) @ (surface_points.T - scale_mat[:3, 3:])).T
    surface_points_camcord = world2cam(surface_points,pose)
    return surface_points_camcord[:,2] , depth_valid
...
def get_surface_point(uv, pose, intrinsics,depth):
    if pose.shape[1] == 7: #In case of quaternion vector representation
        cam_loc = pose[:, 4:]
        R = quat_to_rot(pose[:,:4])
        p = torch.eye(4).repeat(pose.shape[0],1,1).cuda().float()
...
def pixel2cam(uv, pose, intrinsics,depth):
    if pose.shape[1] == 7: #In case of quaternion vector representation
        cam_loc = pose[:, 4:]
        R = quat_to_rot(pose[:,:4])
        p = torch.eye(4).repeat(pose.shape[0],1,1).cuda().float()
    ...
    pixel_points_cam = lift(x_cam, y_cam, z_cam, intrinsics=intrinsics) #[1,1024,4]

    return pixel_points_cam[:,:,:3]
