...
class MonoSDFTrainRunner():
    def __init__(self,**kwargs):
        ...
        self.max_total_iters = self.conf.get_int('train.max_total_iters', default=300000)
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.nepochs + 1):
            ...
            if self.GPU_INDEX == 0 and self.do_vis and epoch % self.plot_freq == 0:
                ...
                plot_data = self.get_plot_data(model_input, model_outputs, model_input['pose'], ground_truth['rgb'], ground_truth['normal'], ground_truth['depth'],ground_truth)
            ...
            for data_index, (indices, model_input, ground_truth) in enumerate(self.train_dataloader):
                ...
                loss_output = self.loss(self.model,model_outputs, ground_truth,model_input)
                ...
                if self.GPU_INDEX == 0:
                    print(
                        '{0}_{1} [{2}] ({3}/{4}): loss = {5}, rgb_loss = {6}, eikonal_loss = {7},sdf_loss={11}, psnr = {8}, bete={9}, alpha={10}'
                    ...
                    self.writer.add_scalar('Loss/sdf_loss', loss_output['sdf_loss'].item(), self.iter_step)
                    self.writer.add_scalar('Loss/surface_loss', loss_output['surface_loss'].item(), self.iter_step)
                    self.writer.add_scalar('Loss/ncc_loss', loss_output['ncc_loss'].item(), self.iter_step)
    ...
    def get_plot_data(self, model_input, model_outputs, pose, rgb_gt, normal_gt, depth_gt,ground_truth):
        loss_output = self.loss(self.model,model_outputs, ground_truth,model_input)
